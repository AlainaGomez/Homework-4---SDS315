...
```{r, echo = FALSE}
boot_rich <- do(10000)*cor(Price ~ Income, data=mosaic::resample(gas_prices))
boot_r <- boot_rich$cor
```

**Evidence**: Boostrapping another sample 10,0000 times, I found the correlation coefficients between local median income and gas prices of certain ZIP codes in Austin. The histogram shows the distribution of these correlation (relationship strength between household income and gas prices) values for the 10,000 samples. With 95% confidence, the true correlation between household income and gas prices is somewhere between `r round(quantile(boot_r, probs = 0.025), 3)` and `r round(quantile(boot_r, probs = 0.975), 3)`. Because correlation coefficient strength is on a scale of 0 to 1, with 1 indicating a strong relationship, it could be said that there is an association and slight relationship between the two as the average correlation is about `r round(mean(boot_rich$cor),3)`.

```{r, echo = FALSE, message = FALSE}
ggplot(boot_rich) + geom_histogram(aes(x=cor), col = "black") + labs(x = "Correlation (0-1)", y = "Count", title = "Correlation of Local Median Income and Gas Prices")
```

**Conclusion**: Generally, correlations greater than 0.5 support the idea of strong relationships between two variables. However, the histogram shows that the true correlation is below this and does not support this ideal. The richer the area (higher median household income) does NOT mean that there will be higher gas prices.
